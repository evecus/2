...
func main() {
	// ── 1. Init encrypted data directory ──────────────────────────────────
	...
	// Restrict CORS to same-origin by default; allow explicit origins via env
...
func printBanner(cfg *config.Config) {
  ...
  👤 Username  : %s
  📦 Version   : %s
  🔐 Config    : AES-256-GCM encrypted (data/config.enc)
